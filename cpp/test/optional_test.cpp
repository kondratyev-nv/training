...
TEST(optional, can_build_optional_from_temporary_value_without_default_constructor) {
...
class equality_dummy {
   public:
    ...
    bool operator==(equality_dummy const& other) const { return x() == other.x() && y() == other.y(); }
...
class not_trivially_destructible_dummy {
   public:
    not_trivially_destructible_dummy(std::function<void()> destructor) : destructor_(destructor) {}
